...
- В **Decap** у каждого занятия поле **«Кто не был или уважительно»**: добавляются только **Н** и **У**; все остальные считаются **«был(а)»** без ручного ввода.
- **Главная** автоматически показывает блок «Ближайшие занятия» из того же `attendance.md` (до 12 дат с сегодняшнего дня).
...
- Однократная миграция старого формата (`records` со всеми учениками → только исключения): `npm run schedule:migrate`.
